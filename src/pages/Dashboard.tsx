...
import { cves, devices, severityHsl } from "@/data/mock";
...
const kpis = [
  { label: "Total Devices", value: devices.length, icon: HardDrive, sub: "+2 this week", tone: "text-foreground" },
  { label: "Critical CVEs", value: cves.filter(c => c.severity === "Critical").length, icon: Flame, sub: "Action required", tone: "text-severity-critical", critical: true },
  { label: "High CVEs", value: cves.filter(c => c.severity === "High").length, icon: ShieldAlert, sub: "Review queue", tone: "text-severity-high" },
  { label: "Patched Today", value: 7, icon: CheckCircle2, sub: "+3 vs yesterday", tone: "text-status-online" },
  { label: "Alerts Sent", value: 24, icon: BellRing, sub: "Last 24h", tone: "text-primary" },
];

const donutData = (["Critical", "High", "Medium", "Low"] as const).map(sev => ({
  name: sev,
  value: cves.filter(c => c.severity === sev).length,
  color: severityHsl(sev),
}));

export default function Dashboard() {
  const totalCves = donutData.reduce((a, b) => a + b.value, 0);
  return (
    <>
      ...
      <div className="p-6 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          {kpis.map(k => (
            <div key={k.label} className={cn("panel p-4 group hover:border-primary/40 transition-all", k.critical && "border-severity-critical/40")}>
          ...
          <div className="panel-glow p-5 scan-line">
            ...
            <div className="relative h-56">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={donutData} dataKey="value" innerRadius={60} outerRadius={88} paddingAngle={3} stroke="hsl(var(--background))" strokeWidth={2}>
                    {donutData.map((d, i) => <Cell key={i} fill={d.color} />)}
                  </Pie>
                  <Tooltip
                    contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", fontSize: 12, fontFamily: "JetBrains Mono" }}
                    cursor={false}
                  />
                </PieChart>
              </ResponsiveContainer>
                ...
                <tbody className="divide-y divide-border">
                  {cves.slice(0, 7).map(c => (
                    <tr key={c.id} className="hover:bg-muted/20 transition-colors">
                      <td className="px-4 py-2.5"><CveLink id={c.id} /></td>
        ...
        <div className="panel p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-[10px] font-mono text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-status-online" /> Healthy</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-status-warning" /> Vulnerable</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-severity-critical" /> Critical</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-status-offline" /> Offline</span>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 gap-2.5">
            {devices.map(d => {
              const dot =
                d.status === "healthy" ? "bg-status-online" :
                d.status === "vulnerable" ? "bg-status-warning" :
                d.status === "critical" ? "bg-severity-critical pulse-critical" : "bg-status-offline";
              const border =
                d.status === "critical" ? "border-severity-critical/40 hover:border-severity-critical" :
                d.status === "vulnerable" ? "border-status-warning/30 hover:border-status-warning" :
                d.status === "offline" ? "border-border opacity-60" : "border-border hover:border-primary/40";
              return (
